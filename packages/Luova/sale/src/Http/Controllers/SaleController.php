...
use Luova\Sale\Http\Requests\SaleSettingFV;
use Luova\Sale\Models\Customer;
...
class SaleController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    ...
                    $button .= '<a data-route="' . route('sale.ajax.delete', $data->id) . '" onclick="isdelete(this)" href="javascript:void(0)" class="btn btn-danger btn-flat"><i class="material-icons">delete_forever</i></a>';
                    $button .= '</div>';
                    return $button;
                })
    ...
    {
        $fdata = SaleSetting::first();
        $mdata = null;
        return view('sale::setting')
    ...
    public function invoice(Request $request, $id)
    {
        ...
        // dd($mdata);
            ...
            // dd($mdata->details()->first()->product);
            $config = [
                'format' => 'A4',
                'margin_header' => 0,
                'margin_footer' => 0,
            ...
            $pdf = PDF::loadView('sale::pdf.invoice', $data, [], $config);
            return $pdf->stream('leaser_pdf' . '.pdf');
        ...
        try {
            ...
            if ($id) {
                ...
                if($request->is_local){
                    ...
                    $attributes['customer_id'] = Customer::create([
                        'name' => $request->cusomer_name,
                        'phone' => $request->Customer_phone,
                        'email' => $request->customer_email,
                        'address' => $request->customer_address,
                        'city' => $request->cusomer_city,
                        'zip' => $request->price_zip,
                    ])->id;
    ...
    public function setting_store(SaleSettingFV $request)
    {

        try {

            $setting = SaleSetting::first();
            if(!$setting){
                $setting = new SaleSetting;
            }
            $setting->cogs = $request->cogs;
            $setting->shipping_fee = $request->shipping_fee;
            $setting->discount = $request->discount;
            $setting->tax_vax = $request->tax_vax;
            $setting->round_of = $request->round_of;
            $setting->save();
