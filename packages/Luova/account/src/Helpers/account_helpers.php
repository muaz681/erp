...
if (!function_exists('AccountSelectType')) {
    function AccountSelectType($type)
    {
       // dd($head);
        $data = Account::select('accounts.*');
    
        $data = $data->join('account_heads', 'account_heads.id', '=', 'accounts.ac_head');
        $data = $data->where('account_heads.type',$type);
        $data = $data->oldest()->get();
