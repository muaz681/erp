                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    {{ Form::label('parent', 'Parrent Account')}}
                                    {{ Form::select('parent', AccountHeadSelect(),(!empty($fdata->parent) ? $fdata->parent : NULL), ['id' => 'parent','class' => (($errors->has('parent')? 'is-invalid': ''). ' form-control select2'), 'placeholder' => 'Select' ]) }}
                                    @error('parent')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                ...
                                <div class="form-group">
                                    {{ Form::label('ac_head', 'Account Head')}}
...
jQuery(document).ready(function($) {
    ...
    $(document).on('change', '#parent', function(e) {
        ...
        if(parent){
            $('#ac_head').prop('disabled', true);
            $('#ac_head').val('');
            $('#ac_head').val('').trigger('change');
            e.preventDefault();

        }

    });
    $(document).on('change', '#ac_head', function(e) {
        var head = $('#ac_head').val();
        if(head){
